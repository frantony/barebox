...
#define PICO_TFTP_DATA_HDR_SIZE  (4)
#define TFTP_HEADER_SIZE	2
...
static void tftp_cb(uint16_t ev, struct pico_socket *sock)
{
	...
	len = pico_socket_recvfrom(sock, pkt, priv->blocksize + PICO_TFTP_DATA_HDR_SIZE, &ep, &remote_port);
