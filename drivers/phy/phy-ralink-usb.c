...
static int ralink_usb_phy_power_on(struct phy *_phy)
{
	...
	u32 t;
		...
		pr_err("ralink_usb_phy_power_on\n");
	/* enable the phy */
...
static int ralink_usb_phy_power_off(struct phy *_phy)
{
	struct ralink_usb_phy *phy = phy_get_drvdata(_phy);
		...
		pr_err("ralink_usb_phy_power_off\n");
	/* disable the phy */
	regmap_update_bits(phy->sysctl, RT_SYSC_REG_CLKCFG1,
			   phy->clk, 0);
...
static struct phy *ralink_usbphy_xlate(struct device_d *dev,
					struct of_phandle_args *args)
{
	struct ralink_usb_phy *phy = dev->priv;
		pr_err("ralink_usbphy_xlate\n");
...
static int ralink_usb_phy_probe(struct device_d *dev)
{
	struct resource *res;
	...
	ret = dev_get_drvdata(dev, (const void **)&phy->clk);
	if (ret) {
		free(phy);
		return ret;
	}
	...
	phy->sysctl = syscon_regmap_lookup_by_phandle(dev->device_node, "ralink,sysctl");
	...
	if (of_device_is_compatible(dev->device_node, "mediatek,mt7628-usbphy")) {
		res = dev_request_mem_resource(dev, 0);
		if (IS_ERR(res))
			return PTR_ERR(res);
		phy->base = IOMEM(res->start);
	}

	phy->rsthost = reset_control_get(dev, "host");
	if (IS_ERR(phy->rsthost)) {
		dev_err(dev, "host reset is missing\n");
		return PTR_ERR(phy->rsthost);
	}

	phy->rstdev = reset_control_get(dev, "device");
	if (IS_ERR(phy->rstdev)) {
		dev_err(dev, "device reset is missing\n");
		return PTR_ERR(phy->rstdev);
	}

	phy->phy = phy_create(dev, NULL, &ralink_usb_phy_ops);
	if (IS_ERR(phy->phy)) {
	...
	phy_provider = of_phy_provider_register(dev, ralink_usbphy_xlate);
...
static struct driver_d ralink_usb_phy_driver = {
	.name  = "ralink-usb-phy",
	.probe	= ralink_usb_phy_probe,
	.of_compatible = ralink_usb_phy_of_match,
};
device_platform_driver(ralink_usb_phy_driver);
