...
/* FIXME: MIPS only */
#define phys_to_bus(x) (0x0fffffff & (dma_addr_t)(x))

/* FIXME: it is dma_alloc_coherent() from arch/mips/lib/dma.c */

	priv->tx_bufs = xmemalign(4096, TX_BUF_TOT_LEN);
	/* ioremap_nocache() */
	priv->tx_bufs = (unsigned char *) (0xa0000000 | (u32)(priv->tx_bufs));
	priv->tx_bufs_dma = phys_to_bus(priv->tx_bufs);

	priv->rx_ring = xmemalign(4096, RX_BUF_TOT_LEN);
	/* ioremap_nocache() */
	priv->rx_ring = (unsigned char *) (0xa0000000 | (u32)(priv->rx_ring));
	priv->rx_ring_dma = phys_to_bus(priv->rx_ring);
