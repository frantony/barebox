...
int pcibios_init()
{
	...
	/* Perform the write */
	GT_WRITE(GT_PCI0_CFGDATA_OFS, 0x1be00000 /* FIXME: virt_to_phys(GT64120_BASE) */);
#endif

	/*
	 Here is linux code. It assumes, that firmware made the work...
	 In barebox this work is done by arch/mips/mach-malta/lowlevel_init.S
	*/
	...
	gt64120_mem_resource.end = res_end;
	...
	res_end <<= GT_PCI_DCRM_SHF;
	res_end |= (1 << GT_PCI_DCRM_SHF) - 1;
	gt64120_io_resource.end = res_end;

#if 0
	/*
	 Can't register "gt64120_io" memory bank just now
	 because of conflict
	  0xb8000000 - 0xb8fffffe (size 0x00ffffff) gt64120_io
          0xb80003f8 - 0xb80003ff (size 0x00000008) ns16550_serial0
	*/
	barebox_add_memory_bank("gt64120_io",
		0xa0000000 | gt64120_io_resource.start,
		gt64120_io_resource.end - gt64120_io_resource.start);
#endif

	// FIXME
	barebox_add_memory_bank("gt64120_mem",
		0xa0000000 | gt64120_mem_resource.start,
		gt64120_mem_resource.end - gt64120_mem_resource.start);
